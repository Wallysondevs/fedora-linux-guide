...
export function CodeBlock({ code, language = "bash", title }: CodeBlockProps) {
  ...
  return (
    <div className="my-6 rounded-xl overflow-hidden bg-[#0d0d0d] border border-border shadow-lg">
      <div className="flex items-center justify-between px-4 py-2 bg-[#1a1a1a] border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          {title ? (
            ...
            <span className="ml-2 text-xs font-mono text-gray-500 lowercase">{language}</span>
          )}
        </div>
        <button
          onClick={handleCopy}
